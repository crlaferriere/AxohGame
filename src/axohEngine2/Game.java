...
import axohEngine2.states.GameMenuTransition;
...
public abstract class Game extends JFrame implements Runnable, KeyListener, MouseListener, MouseMotionListener {
	...
	public void setGameState(State newState) {
		...
		if (currentState != newState) {
			if (currentState != null) {
				gameStates.get(currentState).unload();
			}
			state = newState;
			if (newState != null) {
				gameStates.get(newState).load();
			}
		}
	}
	...
	public Game(int frameRate, int width, int height) {
       ...
       // System.out.println(tk.getScreenInsets(this.getGraphicsConfiguration()));
        ...
        //Start the game
        gameStartUp();
        int offset = (int) ((float) tk.getScreenInsets(this.getGraphicsConfiguration()).bottom * 0.6f) + 2;
